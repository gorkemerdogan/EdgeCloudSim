 ...
 * offloaded to the edge servers. On the other hand, they use LAN
...
public class HangarSimMobileDeviceManager extends MobileDeviceManager {
	...
	protected void processCloudletReturn(SimEvent ev) {
		...
		if(task.getAssociatedDatacenterId() == SimSettings.CLOUD_DATACENTER_ID){
			...
			NETWORK_DELAY_TYPES delayType = NETWORK_DELAY_TYPES.LAN_DELAY;
	...
	protected void processOtherEvent(SimEvent ev) {
		...
		switch (ev.getTag()) {
			...
			{
				((HangarSimNetworkModel)networkModel).updateMM1QueeuModel();
			...
			case RESPONSE_RECEIVED_BY_EDGE_DEVICE_TO_RELAY_MOBILE_DEVICE:
			{
				...
				if(delay > 0)
				{
					...
					if(task.getSubmittedLocation().getServingWlanId() == currentLocation.getServingWlanId())
					{
						...
						SimLogger.getInstance().setDownloadDelay(task.getCloudletId(), delay, NETWORK_DELAY_TYPES.LAN_DELAY);
